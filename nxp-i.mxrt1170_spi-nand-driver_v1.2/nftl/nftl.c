...
int hal_flash_read_with_spare(hal_partition_t in_partition, uint32_t *off_set,
                                 void *data_buf, uint32_t data_buf_len,
                                 void *spare_buf, uint32_t spare_buf_len)
{
    int ret;

    ret = spi_nand_read_oob(EXAMPLE_FLEXSPI, *off_set, buff, data_buf_len + spare_buf_len + TAG_OFFSET);
    if (ret != 0) {
        NFTL_ERROR("[%s] read spare error, off_set: %lx, spare_buf_len: %lx", __func__, *off_set, spare_buf_len);
        return ret;
    }

    memcpy(data_buf, buff, data_buf_len);
    memcpy(spare_buf, buff + NFTL_PAGE_SIZE + TAG_OFFSET, spare_buf_len);

   return ret;
}

int hal_flash_write_with_spare(hal_partition_t in_partition, uint32_t *off_set,
                                 void *data_buf, uint32_t data_buf_len,
                                 void *spare_buf, uint32_t spare_buf_len)
{
    int ret;

    memcpy(buff, data_buf, data_buf_len);
    buff[NFTL_PAGE_SIZE] = 0xFF;
    memcpy(buff + NFTL_PAGE_SIZE + TAG_OFFSET, spare_buf, spare_buf_len);

    ret = spi_nand_write_oob(EXAMPLE_FLEXSPI, *off_set, buff, data_buf_len + spare_buf_len + TAG_OFFSET);
...
int hal_flash_isbad(hal_partition_t in_partition, uint32_t off_set)
{
    ...
    ret = spi_nand_read_oob(EXAMPLE_FLEXSPI, addr, &bad_flag0, 1);
    if (ret != 0) {
    ...
    ret = spi_nand_read_oob(EXAMPLE_FLEXSPI, addr, &bad_flag1, 1);
    if (ret != 0) {
        NFTL_ERROR("[%s] read error, address: %lx, len: 1", __func__, addr);
        return ret;
    }
...
int hal_flash_markbad(hal_partition_t in_partition, uint32_t off_set)
{
    ...
    addr = (off_set / NFTL_BLOCK_OFFSET * NFTL_BLOCK_OFFSET) + NFTL_PAGE_SIZE + BAD_FLAG_OFFSET;
    ret = spi_nand_write_oob(EXAMPLE_FLEXSPI, addr, &bad_flag0, 1);
    if (ret != 0) {
    ...
    addr = (off_set / NFTL_BLOCK_OFFSET * NFTL_BLOCK_OFFSET) + NFTL_PAGE_SIZE * 2 + BAD_FLAG_OFFSET;
    ret = spi_nand_write_oob(EXAMPLE_FLEXSPI, addr, &bad_flag1, 1);
    if (ret != 0) {
        NFTL_ERROR("[%s] program error, address: %lx, len: 1", __func__, addr);
        return ret;
    }
...
//        for (int i = 0; i < 16; i++) {
//          if(blk_mgr->spare_buff[i] != 0xFF)
//              printf("blk_mgr->spare_buff: %ld\r\n",blk_mgr->spare_buff[i]);
//
//        }
...
static void nftl_blk_release(nftl_block_mgr_t *blk_mgr ,uint32_t phy_blk)
{
    ...
    if (0 == count_valid_page(blk_mgr, phy_blk)) {
    	ret = spi_nand_erase(EXAMPLE_FLEXSPI, phy_blk * NFTL_BLOCK_OFFSET, NFTL_BLOCK_SIZE);
        if (ret != 0) {
    ...
    do {
        ...
        for (i = 0; i < NFTL_TAG_PAGE_IN_BLOCK; i++) {
            if (nftl_isvalid_bitmap(blk_mgr->phy_blocks[old_blk].page_bitmap, i)) {
                NFTL_ASSERT(r_off == old_blk * NFTL_BLOCK_OFFSET + i * NFTL_PAGE_SIZE);
...
//                r_off += NFTL_PAGE_SIZE;
    ...
    ret = spi_nand_erase(EXAMPLE_FLEXSPI, old_blk * NFTL_BLOCK_OFFSET, NFTL_BLOCK_SIZE);
    if (ret != 0) {
...
static void nftl_update_tagpage(nftl_block_mgr_t *blk_mgr, uint32_t blk)
{
    ...
    do {
        ...
        /* TODO: deal with endian */
        ret = spi_nand_write(EXAMPLE_FLEXSPI, w_off, blk_tags, sizeof(nftl_block_tag) * NFTL_TAG_PAGE_IN_BLOCK);
...
static uint32_t nftl_alloc_new_block(nftl_block_mgr_t *blk_mgr)
{
    ...
    do {
        ...
        ret = spi_nand_erase(EXAMPLE_FLEXSPI, offset, NFTL_BLOCK_SIZE);
        if (ret) {
...
static nftl_err_t nftl_init_partition(nftl_block_mgr_t *blk_mgr)
{
    ...
    for (i = 0; i < blk_mgr->phy_block_nums; i++) {
        ...
        /* TODO: deal with endian */
        ret = spi_nand_read(EXAMPLE_FLEXSPI, r_off, blk_mgr->page_buff, NFTL_PAGE_SIZE);
        r_off += NFTL_PAGE_OFFSET;
                    ...
                    void *outbuf, nftl_size_t len)
{
    ...
    while (len > 0) {
        ...
        /* TODO: deal with endian */
        ret = spi_nand_read(EXAMPLE_FLEXSPI, r_off, p_buf, r_len);
        r_off += r_len;
...
void nftl_init(void)
{
 ...
 //  NFTL_WARN("%s end complethsdfgsdfgtrye gc, part: %d, empty blocks: %d\r\n", __func__, 1, 10);
   ...
   if (!g_block_mgr[NFTL_PARTITION0].initialized)
   {
       ...
       //printf("logic_block_nums %ld phy_block_nums %d\r\n", NFTL_TAG_PAGE_IN_BLOCK * NFTL_PART0_BLOCK_NUMS ,NFTL_PART0_BLOCK_NUMS + NFTL_PART0_BUFFER_BLOCK_NUMS);
   #if NFTL_LOGICAL_PARTITION_NUM > 1
...
int nftl_flash_eraseall(nftl_partition_t p)
{
    ...
    for (uint32_t blk = 0; blk < blk_mgr->phy_block_nums; blk++) {
    	ret = spi_nand_erase(EXAMPLE_FLEXSPI, blk * NFTL_BLOCK_OFFSET, NFTL_BLOCK_SIZE);
        if (ret != 0) {
